layui.use(['layer', 'ax', 'form', 'laydate', 'element', 'table'], function () {
    ...
    $('#execute').on('click', function () {
        ...
        if (srcPath) {
            console.log("生成到指定位置:" + srcPath)
            srcPath = srcPath.replace(/\\/g, ".");
            var ajax = new $ax(Feng.ctxPath + "/execute?dataSourceId=" + dataSourceId + "&author="
                + author + "&proPackage=" + proPackage + "&removePrefix=" + removePrefix + "&tables=" + Code.tableNames + "&srcPath=" + srcPath,
                function () {
                    Feng.success("生成成功!");
                }, function () {
                    layer.msg("生成成功！");
